...
def make_new_dset(parent, shape=None, dtype=None, data=None,
    ...
    if shape is None:
        ...
        shape = tuple(shape)
        if data is not None and (numpy.product(shape, dtype=numpy.uint64) != numpy.product(data.shape, dtype=numpy.uint64)):
    ...
    def __getitem__(self, args):
        ...
        * Boolean "mask" array indexing
        ...
        if len(args) == 1 and isinstance(args[0], h5r.RegionReference):
            ...
            if numpy.product(mshape, dtype=numpy.uint64) == 0:
        ...
        if numpy.product(self.shape, dtype=numpy.uint64) == 0:
            ...
            if vlen == val.dtype:
                if val.ndim > 1:
                    ...
                    tmp.ravel()[:] = [i for i in val.reshape(
                        (numpy.product(val.shape[:-1], dtype=numpy.uint64), val.shape[-1]))]
                else:
