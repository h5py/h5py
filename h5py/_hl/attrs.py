    ...
    def create(self, name, data, shape=None, dtype=None):
        ...
        with phil:
            ...
            # is compatible, and reshape if needed.
                ...
                if shape is not None and numpy.product(shape, dtype=numpy.uint64) != numpy.product(data.shape, dtype=numpy.uint64):
        ...
        with phil:
            if not name in self:
                ...
                if (value.shape != attr.shape) and not \
                   (numpy.product(value.shape, dtype=numpy.uint64) == 1 and numpy.product(attr.shape, dtype=numpy.uint64) == 1):
