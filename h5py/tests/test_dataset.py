...
class TestCreateShape(BaseDataset):
    ...
    def test_create_extended_1d(self):
        ...
        dset = self.f.create_dataset(name(), (63,))
    ...
    def test_create_extended_2d(self):
        ...
        dset = self.f.create_dataset(name(), (6, 10))
    ...
    def test_long_double(self):
        """ Confirm that the default dtype is float """
        dset = self.f.create_dataset(name(), (63,), dtype=np.longdouble)
...
class TestCreateData(BaseDataset):
    ...
    def test_reshape(self):
        ...
        dset = self.f.create_dataset(name(), shape=(10, 3), data=data)
    ...
    def test_create_bytestring(self):
        ...
        check_vlen_ascii(self.f.create_dataset(name("a"), data=b'abc'))
        check_vlen_ascii(self.f.create_dataset(name("b"), data=[b'abc', b'def']))
        check_vlen_ascii(self.f.create_dataset(name("c"), data=[[b'abc'], [b'def']]))
        check_vlen_ascii(self.f.create_dataset(
            name("d"), data=np.array([b'abc', b'def'], dtype=object)
        ))

    def test_create_np_s(self):
        dset = self.f.create_dataset(name(), data=np.array([b'abc', b'def'], dtype='S3'))
    ...
    def test_create_strings(self):
        ...
        check_vlen_utf8(self.f.create_dataset(name("a"), data='abc'))
        check_vlen_utf8(self.f.create_dataset(name("b"), data=['abc', 'def']))
        check_vlen_utf8(self.f.create_dataset(name("c"), data=[['abc'], ['def']]))
        check_vlen_utf8(self.f.create_dataset(
            name("d"), data=np.array(['abc', 'def'], dtype=object)
        ))
    ...
    def test_create_incompatible_data(self):
        ...
        with self.assertRaises(ValueError):
            self.f.create_dataset(name(), shape=4, data= np.arange(3))
...
class TestReadDirectly:
    ...
    def test_empty(self, writable_file):
        empty_dset = writable_file.create_dataset(name(), dtype='int64')
...
class TestCreateChunked(BaseDataset):
    ...
    def test_create_chunks(self):
        ...
        dset = self.f.create_dataset(name(), shape=(100,), chunks=(10,))
    ...
    def test_auto_chunks_abuse(self):
        ...
        dset = self.f.create_dataset(name(), shape=(3,), dtype='S100000000', chunks=True)
        self.assertEqual(dset.chunks, (1,))

    def test_scalar_assignment(self):
        """ Test scalar assignment of chunked dataset """
        dset = self.f.create_dataset(name(), shape=(3, 50, 50),
...
class TestCreateFillvalue(BaseDataset):
    ...
    def test_create_fillval(self):
        ...
        dset = self.f.create_dataset(name(), (10,), fillvalue=4.0)
        self.assertEqual(dset[0], 4.0)
        self.assertEqual(dset[7], 4.0)

    def test_property(self):
        """ Fill value is recoverable via property """
        dset = self.f.create_dataset(name(), (10,), fillvalue=3.0)
    ...
    def test_compound(self):
        ...
        dset = self.f.create_dataset(name(), (10,), dtype=dt, fillvalue=v)
...
class TestFillTime(BaseDataset):
    ...
    def test_compressed_default(self):
        ...
        dset = self.f.create_dataset(name(), (10,), compression='gzip',
                                     fillvalue=4.0)
        plist = dset.id.get_create_plist()
        self.assertEqual(plist.get_fill_time(), h5py.h5d.FILL_TIME_IFSET)
        self.assertEqual(dset[0], 4.0)
    ...
    def test_fill_time_ifset(self):
        ...
        dset = self.f.create_dataset(name(), (10,), chunks=(2,), fillvalue=4.0,
                                     fill_time='ifset')
        plist = dset.id.get_create_plist()
        self.assertEqual(plist.get_fill_time(), h5py.h5d.FILL_TIME_IFSET)
...
def test_get_unset_fill_value(dt, expected, writable_file):
    dset = writable_file.create_dataset(name(), (10,), dtype=dt)
...
class TestCreateNamedType(BaseDataset):
    ...
    def test_named(self):
        ...
        self.f[name("type")] = np.dtype('f8')
        dt = self.f[name("type")]
        dset = self.f.create_dataset(name("x"), (100,), dtype=dt)
...
class TestCreateGzip(BaseDataset):
    ...
    def test_gzip(self):
        ...
        dset = self.f.create_dataset(name(), (20, 30), compression='gzip',
                                     compression_opts=9)
...
class TestCreateLZF(BaseDataset):
    ...
    def test_lzf_sparse(self):
        ...
        dset = self.f.create_dataset(name(), (20, 30), compression='lzf')
    ...
    def test_lzf_exc(self):
        ...
        with self.assertRaises(ValueError):
            self.f.create_dataset(name(), (20, 30), compression='lzf',
                                  compression_opts=4)
...
class TestCreateSZIP(BaseDataset):
    ...
    def test_szip(self):
        ...
        dset = self.f.create_dataset(name(), (20, 30), compression='szip',
...
class TestCreateShuffle(BaseDataset):
    ...
    def test_shuffle(self):
        ...
        dset = self.f.create_dataset(name(), (20, 30), shuffle=True)
...
class TestCreateFletcher32(BaseDataset):
    ...
    def test_fletcher32(self):
        ...
        dset = self.f.create_dataset(name(), (20, 30), fletcher32=True)
...
class TestCreateScaleOffset(BaseDataset):
    ...
    def test_float_fails_without_options(self):
        ...
        with self.assertRaises(ValueError):
            dset = self.f.create_dataset(name(), (20, 30), dtype=float, scaleoffset=True)

    def test_non_integer(self):
        """ Check when scaleoffset is negetive"""

        with self.assertRaises(ValueError):
            dset = self.f.create_dataset(name(), (20, 30), dtype=float, scaleoffset=-0.1)

    def test_unsupport_dtype(self):
        """ Check when dtype is unsupported type"""

        with self.assertRaises(TypeError):
            dset = self.f.create_dataset(name(), (20, 30), dtype=bool, scaleoffset=True)
...
class TestExternal(BaseDataset):
    ...
    def test_contents(self):
        ...
        dset = self.f.create_dataset(name(), shape, dtype=testdata.dtype, external=external, efile_prefix="${ORIGIN}")
        dset[...] = testdata
    ...
    def test_name_str(self):
        ...
        self.f.create_dataset(name(), (6, 100), external=self.mktemp())

    def test_name_path(self):
        """ External argument may be a file name path only """

        self.f.create_dataset(name(), (6, 100),
    ...
    def test_iter_multi(self):
        ...
        dset = self.f.create_dataset(name(), (6, 100), external=external)
    ...
    def test_invalid(self):
        ...
        for exc_type, external in [
            ...
            with self.assertRaises(exc_type):
                self.f.create_dataset(name(), shape, external=external)
    ...
    def test_create_expandable(self):
        ...
        shape = (128, 64)
        maxshape = (None, 64)
        exp_dset = self.f.create_dataset(name(), shape=shape, maxshape=maxshape,
                                         external=ext_file)
...
class TestAutoCreate(BaseDataset):
    ...
    def test_vlen_bytes(self):
        ...
        self.f[name("x")] = b"Hello there"
        self.assert_string_type(self.f[name("x")], h5py.h5t.CSET_ASCII)

        self.f[name("y")] = [b"a", b"bc"]
        self.assert_string_type(self.f[name("y")], h5py.h5t.CSET_ASCII)

        self.f[name("z")] = np.array([b"a", b"bc"], dtype=np.object_)
        self.assert_string_type(self.f[name("z")], h5py.h5t.CSET_ASCII)

    def test_vlen_unicode(self):
        """Assigning unicode strings produces a vlen string UTF-8 dataset """
        self.f[name("x")] = "Hello there" + chr(0x2034)
        self.assert_string_type(self.f[name("x")], h5py.h5t.CSET_UTF8)

        self.f[name("y")] = ["a", "bc"]
        self.assert_string_type(self.f[name("y")], h5py.h5t.CSET_UTF8)

        # 2D array; this only works with an array, not nested lists
        self.f[name("z")] = np.array([["a", "bc"]], dtype=np.object_)
        self.assert_string_type(self.f[name("z")], h5py.h5t.CSET_UTF8)
...
class TestCreateLike(BaseDataset):
    def test_no_chunks(self):
        self.f[name("x")] = np.arange(25).reshape(5, 5)
        self.f.create_dataset_like(name("y"), self.f[name("x")])
        dslike = self.f[name("y")]
        self.assertEqual(dslike.shape, (5, 5))
        self.assertIs(dslike.chunks, None)

    def test_track_times(self):
        orig = self.f.create_dataset(name("x"), data=np.arange(12),
                                     track_times=True)
        self.assertNotEqual(0, h5py.h5g.get_objinfo(orig._id).mtime)
        similar = self.f.create_dataset_like(name("y"), orig)
        self.assertNotEqual(0, h5py.h5g.get_objinfo(similar._id).mtime)

        orig = self.f.create_dataset(name("z"), data=np.arange(12),
                                     track_times=False)
        self.assertEqual(0, h5py.h5g.get_objinfo(orig._id).mtime)
        similar = self.f.create_dataset_like(name("w"), orig)
        self.assertEqual(0, h5py.h5g.get_objinfo(similar._id).mtime)

    def test_maxshape(self):
        """ Test when other.maxshape != other.shape """

        other = self.f.create_dataset(name("x"), (10,), maxshape=20)
        similar = self.f.create_dataset_like(name("y"), other)
...
class TestChunkIterator(BaseDataset):
    ...
    def test_1d(self):
        dset = self.f.create_dataset(name(), (100,), chunks=(32,))
    ...
    def test_2d(self):
        dset = self.f.create_dataset(name(), (100,100), chunks=(32,64))
        expected = (
    ...
    def test_2d_partial_slice(self):
        dset = self.f.create_dataset(name(), (5,5), chunks=(2,2))
...
class TestResize(BaseDataset):
    ...
    def test_create(self):
        ...
        dset = self.f.create_dataset(name(), (20, 30), maxshape=(20, 60))
    ...
    def test_resize_1D(self):
        ...
        dset = self.f.create_dataset(name(), 20, maxshape=40)
    ...
    def test_resize_nonchunked(self):
        ...
        dset = self.f.create_dataset(name(), (20, 30))
        with self.assertRaises(TypeError):
            dset.resize((20, 60))
    ...
    def test_zero_dim(self):
        ...
        dset = self.f.create_dataset(name(), (15, 0), maxshape=(15, None))
...
class TestDtype(BaseDataset):
    ...
    def test_dtype(self):
        ...
        dset = self.f.create_dataset(name(), (5,), '|S10')
    ...
    def test_dtype_complex32(self):
        ...
        dset = self.f.create_dataset(name(), (5,), complex32)
...
class TestLen(BaseDataset):
    ...
    def test_len(self):
        ...
        dset = self.f.create_dataset(name(), (312, 15))
        self.assertEqual(len(dset), 312)

    def test_len_big(self):
        """ Python len() vs Dataset.len() """
        dset = self.f.create_dataset(name(), (2 ** 33, 15))
...
class TestIter(BaseDataset):
    ...
    def test_iter_scalar(self):
        ...
        dset = self.f.create_dataset(name(), shape=())
        with self.assertRaises(TypeError):
            [x for x in dset]
...
class TestStrings(BaseDataset):
    ...
    def test_vlen_bytes(self):
        ...
        ds = self.f.create_dataset(name(), (100,), dtype=dt)
        tid = ds.id.get_type()
        self.assertEqual(type(tid), h5py.h5t.TypeStringID)
    ...
    def test_vlen_bytes_fillvalue(self):
        ...
        dt = h5py.string_dtype(encoding='ascii')
        fill_value = b'bar'
        ds = self.f.create_dataset(name(), (100,), dtype=dt, fillvalue=fill_value)
    ...
    def test_retrieve_vlen_unicode(self):
        dt = h5py.string_dtype()
        ds = self.f.create_dataset(name(), (10,), dtype=dt)
        data = "fàilte"
        ds[0] = data
    ...
    def test_asstr(self):
        ds = self.f.create_dataset(name(), (10,), dtype=h5py.string_dtype())
        data = "fàilte"
        ds[0] = data
...
class TestSubarray(BaseDataset):
    def test_write_list(self):
        ds = self.f.create_dataset(name(), (1,), dtype="3int8")
        ds[0] = [1, 2, 3]
...
class TestEnum(BaseDataset):
    ...
    def test_create(self):
        ...
        ds = self.f.create_dataset(name(), (100, 100), dtype=dt)
        dt2 = ds.dtype
...
class TestFloats(BaseDataset):
    ...
    def _exectest(self, dt):
        dset = self.f.create_dataset(name(), (100,), dtype=dt)
...
class TestTrackTimes(BaseDataset):
    ...
    def test_disable_track_times(self):
        ...
        ds = self.f.create_dataset(name(), (4,), track_times=False)
    ...
    def test_invalid_track_times(self):
        ...
        with self.assertRaises(TypeError):
            self.f.create_dataset(name(), (4,), track_times='null')
...
class TestZeroShape(BaseDataset):
    ...
    def test_array_conversion(self):
        ...
        ds = self.f.create_dataset(name("x"), 0, maxshape=None)
        self.assertEqual(ds.shape, np.array(ds).shape)

        ds = self.f.create_dataset(name("y"), (0,), maxshape=(None,))
        self.assertEqual(ds.shape, np.array(ds).shape)

        ds = self.f.create_dataset(name("z"), (0, 0), maxshape=(None, None))
        self.assertEqual(ds.shape, np.array(ds).shape)

    def test_reading(self):
        """ Slicing into empty datasets works correctly """
        dt = [('a', 'f'), ('b', 'i')]
        ds = self.f.create_dataset(name(), (0,), dtype=dt, maxshape=(None,))
...
class TestRegionRefs(BaseDataset):
    ...
    def test_scalar_dataset(self):
        ds = self.f.create_dataset(name(), data=1.0, dtype='f4')
...
class TestAstype(BaseDataset):
    ...
    def test_astype_wrapper(self):
        dset = self.f.create_dataset(name(), (100,), dtype='i2')
        dset[...] = np.arange(100)
        arr = dset.astype('f4')[:]
...
class TestScalarCompound(BaseDataset):
    ...
    def test_scalar_compound(self):
        ...
        dt = np.dtype([('a', 'i')])
        dset = self.f.create_dataset(name(), (), dtype=dt)
...
class TestVlen(BaseDataset):
    def test_int(self):
        ...
        ds = self.f.create_dataset(name(), (4,), dtype=dt)
        ds[0] = np.arange(3)
        ds[1] = np.arange(0)
        ds[2] = [1, 2, 3]
        ds[3] = np.arange(1)
    ...
    def test_reuse_from_other(self):
        dt = h5py.vlen_dtype(int)
        ds = self.f.create_dataset(name("x"), (1,), dtype=dt)
        self.f.create_dataset(name("y"), (1,), ds[()].dtype)
    ...
    def test_convert(self):
        dt = h5py.vlen_dtype(int)
        ds = self.f.create_dataset(name(), (3,), dtype=dt)
    ...
    def test_multidim(self):
        dt = h5py.vlen_dtype(int)
        ds = self.f.create_dataset(name(), (2, 2), dtype=dt)
        ds[0, 0] = np.arange(1)
    ...
    def _help_float_testing(self, np_dt):
        ...
        ds = self.f.create_dataset(name(), (5,), dtype=dt)
...
class TestLowOpen(BaseDataset):
    ...
    def test_dapl(self):
        ...
        del dset
        dsid = h5py.h5d.open(self.f.id, bytes(name(), 'utf-8'), dapl)
...
def test_empty_shape(writable_file):
    ds = writable_file.create_dataset(name(), dtype='int32')
...
def test_python_int_uint64(writable_file):
    ...
    ds = writable_file.create_dataset(name(), data=data, dtype=np.uint64)
...
def test_setitem_fancy_indexing(writable_file):
    ...
    arr = writable_file.create_dataset(name(), (5, 1000, 2), dtype=np.uint8)
...
def test_allow_unknown_filter(writable_file):
    ...
    ds = writable_file.create_dataset(
        name(), shape=(10, 10), dtype=np.uint8, compression=fake_filter_id,
...
class TestCommutative(BaseDataset):
    ...
    def test_numpy_commutative(self,):
        ...
        """
        shape = (100,1)
        dset = self.f.create_dataset(name(), shape, dtype=float,
...
class TestVirtualPrefix(BaseDataset):
    ...
    def test_virtual_prefix_require(self):
        virtual_prefix = "/path/to/virtual"
        dset = self.f.require_dataset(name(), (10, 3), 'f', virtual_prefix = virtual_prefix)
        virtual_prefix_readback = pathlib.Path(dset.id.get_access_plist().get_virtual_prefix().decode()).as_posix()
...
def ds_str(file, shape=(10, )):
    dt = h5py.string_dtype(encoding='ascii')
    fill_value = b'fill'
    return file.create_dataset(name(), shape, dtype=dt, fillvalue=fill_value)
