...
class TestRepr(BaseDataset):
    ...
    def test_repr_basic(self):
        ds = self.f.create_dataset(uid:=str(uuid4()), (4,), dtype='int32')
        assert repr(ds) == f'<HDF5 dataset "{uid}": shape (4,), type "<i4">'

    def test_repr_closed(self):
        """ repr() works on live and dead datasets """
        ds = self.f.create_dataset(str(uuid4()), (4,))
        self.f.close()
...
class TestCreateShape(BaseDataset):
    ...
    def test_create_scalar(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), ())
    ...
    def test_create_extended(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), (63,))
        self.assertEqual(dset.shape, (63,))
        self.assertEqual(dset.size, 63)
        dset = self.f.create_dataset(str(uuid4()), (6, 10))
    ...
    def test_missing_shape(self):
        ...
        with self.assertRaises(TypeError):
            self.f.create_dataset(str(uuid4()))

    def test_long_double(self):
        """ Confirm that the default dtype is float """
        dset = self.f.create_dataset(str(uuid4()), (63,), dtype=np.longdouble)
...
class TestCreateData(BaseDataset):
    ...
    def test_create_scalar(self):
        ...
        data = np.ones((), 'f')
        dset = self.f.create_dataset(str(uuid4()), data=data)
    ...
    def test_dataset_intermediate_group(self):
        ...
        ds = self.f.create_dataset(uid:=f"/{uuid4()}/bar/baz", shape=(10, 10), dtype='<i4')
        self.assertIsInstance(ds, h5py.Dataset)
        self.assertTrue(uid in self.f)

    def test_reshape(self):
        """ Create from existing data, and make it fit a new shape """
        data = np.arange(30, dtype='f')
        dset = self.f.create_dataset(str(uuid4()), shape=(10, 3), data=data)
    ...
    def test_create_bytestring(self):
        ...
        check_vlen_ascii(self.f.create_dataset(str(uuid4()), data=b'abc'))
        check_vlen_ascii(self.f.create_dataset(str(uuid4()), data=[b'abc', b'def']))
        check_vlen_ascii(self.f.create_dataset(str(uuid4()), data=[[b'abc'], [b'def']]))
        check_vlen_ascii(self.f.create_dataset(
            str(uuid4()), data=np.array([b'abc', b'def'], dtype=object)
        ))
    ...
    def test_create_strings(self):
        ...
        check_vlen_utf8(self.f.create_dataset(str(uuid4()), data='abc'))
        check_vlen_utf8(self.f.create_dataset(str(uuid4()), data=['abc', 'def']))
        check_vlen_utf8(self.f.create_dataset(str(uuid4()), data=[['abc'], ['def']]))
        check_vlen_utf8(self.f.create_dataset(
            str(uuid4()), data=np.array(['abc', 'def'], dtype=object)
        ))
    ...
    def test_empty_create_via_None_shape(self):
        self.f.create_dataset(uid:=str(uuid4()), dtype='f')
        self.assertTrue(is_empty_dataspace(self.f[uid].id))

    def test_empty_create_via_Empty_class(self):
        self.f.create_dataset(uid:=str(uuid4()), data=h5py.Empty(dtype='f'))
        self.assertTrue(is_empty_dataspace(self.f[uid].id))
...
class TestReadDirectly:
    ...
    def test_read_direct(self, writable_file, source_shape, dest_shape, source_sel, dest_sel):
        ...
        dset = writable_file.create_dataset(str(uuid4()), source_shape, data=source_values)
    ...
    def test_no_sel(self, writable_file):
        dset = writable_file.create_dataset(str(uuid4()), (10,), data=np.arange(10, dtype="int64"))
    ...
    def test_empty(self, writable_file):
        empty_dset = writable_file.create_dataset(str(uuid4()), dtype='int64')
        arr = np.ones((100,), 'int64')
        with pytest.raises(TypeError):
            empty_dset.read_direct(arr, np.s_[0:10], np.s_[50:60])

    def test_wrong_shape(self, writable_file):
        dset = writable_file.create_dataset(str(uuid4()), (100,), dtype='int64')
        arr = np.ones((200,))
        with pytest.raises(TypeError):
            dset.read_direct(arr)

    def test_not_c_contiguous(self, writable_file):
        dset = writable_file.create_dataset(str(uuid4()), (10, 10), dtype='int64')
    ...
    def test_zero_length(self, writable_file):
        shape = (0, 20)
        dset = writable_file.create_dataset(str(uuid4()), shape, dtype=np.int64)
...
class TestWriteDirectly:
    ...
    def test_write_direct(self, writable_file, source_shape, dest_shape, source_sel, dest_sel):
        dset = writable_file.create_dataset(str(uuid4()), dest_shape, dtype='int32', fillvalue=-1)
...
class TestCreateRequire(BaseDataset):
    ...
    def test_shape_conflict(self):
        ...
        self.f.create_dataset(uid:=str(uuid4()), (10, 3), 'f')
        with self.assertRaises(TypeError):
            self.f.require_dataset(uid, (10, 4), 'f')

    def test_type_conflict(self):
        """ require_dataset with object type conflict yields TypeError """
        self.f.create_group(uid:=str(uuid4()))
        with self.assertRaises(TypeError):
            self.f.require_dataset(uid, (10, 3), 'f')

    def test_dtype_conflict(self):
        """ require_dataset with dtype conflict (strict mode) yields TypeError
        """
        dset = self.f.create_dataset(uid:=str(uuid4()), (10, 3), 'f')
        with self.assertRaises(TypeError):
            self.f.require_dataset(uid, (10, 3), 'S10')

    def test_dtype_exact(self):
        """ require_dataset with exactly dtype match """

        dset = self.f.create_dataset(uid:=str(uuid4()), (10, 3), 'f')
        dset2 = self.f.require_dataset(uid, (10, 3), 'f', exact=True)
        self.assertEqual(dset, dset2)

    def test_dtype_close(self):
        """ require_dataset with convertible type succeeds (non-strict mode)
        """
        dset = self.f.create_dataset(uid:=str(uuid4()), (10, 3), 'i4')
        dset2 = self.f.require_dataset(uid, (10, 3), 'i2', exact=False)
        self.assertEqual(dset, dset2)
...
class TestCreateChunked(BaseDataset):
    ...
    def test_create_chunks(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), shape=(100,), chunks=(10,))
    ...
    def test_chunks_mismatch(self):
        ...
        with self.assertRaises(ValueError):
            self.f.create_dataset(str(uuid4()), shape=(100,), chunks=(200,))

    def test_chunks_false(self):
        """ Chunked format required for given storage options """
        with self.assertRaises(ValueError):
            self.f.create_dataset(str(uuid4()), shape=(10,), maxshape=100, chunks=False)

    def test_chunks_scalar(self):
        """ Attempting to create chunked scalar dataset raises TypeError """
        with self.assertRaises(TypeError):
            self.f.create_dataset(str(uuid4()), shape=(), chunks=(50,))

    def test_auto_chunks(self):
        """ Auto-chunking of datasets """
        dset = self.f.create_dataset(str(uuid4()), shape=(20, 100), chunks=True)
    ...
    def test_auto_chunks_abuse(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), shape=(3,), dtype='S100000000', chunks=True)
        self.assertEqual(dset.chunks, (1,))

    def test_scalar_assignment(self):
        """ Test scalar assignment of chunked dataset """
        dset = self.f.create_dataset(str(uuid4()), shape=(3, 50, 50),
    ...
    def test_auto_chunks_no_shape(self):
        ...
        with pytest.raises(TypeError, match='Empty') as err:
            self.f.create_dataset(str(uuid4()), dtype='S100', chunks=True)

        with pytest.raises(TypeError, match='Empty') as err:
            self.f.create_dataset(str(uuid4()), dtype='S100', maxshape=20)
...
class TestCreateFillvalue(BaseDataset):
    ...
    def test_create_fillval(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), (10,), fillvalue=4.0)
        self.assertEqual(dset[0], 4.0)
        self.assertEqual(dset[7], 4.0)

    def test_property(self):
        """ Fill value is recoverable via property """
        dset = self.f.create_dataset(str(uuid4()), (10,), fillvalue=3.0)
    ...
    def test_property_none(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), (10,))
    ...
    def test_compound(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), (10,), dtype=dt, fillvalue=v)
...
class TestFillTime(BaseDataset):
    ...
    def test_compressed_default(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), (10,), compression='gzip',
                                     fillvalue=4.0)
        plist = dset.id.get_create_plist()
        self.assertEqual(plist.get_fill_time(), h5py.h5d.FILL_TIME_IFSET)
        self.assertEqual(dset[0], 4.0)
    ...
    def test_fill_time_ifset(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), (10,), chunks=(2,), fillvalue=4.0,
                                     fill_time='ifset')
        plist = dset.id.get_create_plist()
        self.assertEqual(plist.get_fill_time(), h5py.h5d.FILL_TIME_IFSET)
    ...
    def test_invalid_fill_time(self):
        ...
        with self.assertRaises(ValueError):
            dset = self.f.create_dataset(str(uuid4()), (10,), fill_time='fill_bad')

    def test_non_str_fill_time(self):
        """ fill_time must be a string """
        with self.assertRaises(ValueError):
            dset = self.f.create_dataset(str(uuid4()), (10,), fill_time=2)

    def test_resize_chunk_fill_time_default(self):
        """ The resize dataset will be filled (by default fill value 0) """
        dset = self.f.create_dataset(str(uuid4()), (50, ), maxshape=(100, ),
                                     chunks=(5, ))
        plist = dset.id.get_create_plist()
        self.assertEqual(plist.get_fill_time(), h5py.h5d.FILL_TIME_IFSET)
...
def test_get_unset_fill_value(dt, expected, writable_file):
    dset = writable_file.create_dataset(str(uuid4()), (10,), dtype=dt)
...
class TestCreateNamedType(BaseDataset):
    ...
    def test_named(self):
        ...
        self.f[(uid:=str(uuid4()))] = np.dtype('f8')
        dset = self.f.create_dataset(str(uuid4()), (100,), dtype=self.f[uid])
        self.assertEqual(dset.dtype, np.dtype('f8'))
        self.assertEqual(dset.id.get_type(), self.f[uid].id)
...
class TestCreateGzip(BaseDataset):
    ...
    def test_gzip(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), (20, 30), compression='gzip',
                                     compression_opts=9)
    ...
    def test_gzip_number(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), (20, 30), compression=7)
        self.assertEqual(dset.compression, 'gzip')
    ...
    def test_gzip_exc(self):
        ...
        with self.assertRaises((ValueError, RuntimeError)):
            self.f.create_dataset(str(uuid4()), (20, 30), compression=14)
        with self.assertRaises(ValueError):
            self.f.create_dataset(str(uuid4()), (20, 30), compression=-4)
        with self.assertRaises(ValueError):
            self.f.create_dataset(str(uuid4()), (20, 30), compression='gzip',
                                  compression_opts=14)
...
class TestCreateCompressionNumber(BaseDataset):
    ...
    def test_compression_number(self):
        ...
        try:
            h5py._hl.dataset._LEGACY_GZIP_COMPRESSION_VALS = tuple()
            dset = self.f.create_dataset(str(uuid4()), (20, 30), compression=h5py.h5z.FILTER_DEFLATE, compression_opts=(7,))
        finally:
            h5py._hl.dataset._LEGACY_GZIP_COMPRESSION_VALS = original_compression_vals
    ...
    def test_compression_number_invalid(self):
        ...
        with self.assertRaises(ValueError) as e:
            self.f.create_dataset(str(uuid4()), (20, 30), compression=-999)
        self.assertIn("Invalid filter", str(e.exception))

        with self.assertRaises(ValueError) as e:
            self.f.create_dataset(str(uuid4()), (20, 30), compression=100)
        ...
        try:
            ...
            with self.assertRaises(IndexError):
                self.f.create_dataset(str(uuid4()), (20, 30), compression=h5py.h5z.FILTER_DEFLATE)
        finally:
            h5py._hl.dataset._LEGACY_GZIP_COMPRESSION_VALS = original_compression_vals
...
class TestCreateLZF(BaseDataset):
    ...
    def test_lzf(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), (20, 30), compression='lzf')
        self.assertEqual(dset.compression, 'lzf')
        self.assertEqual(dset.compression_opts, None)

        testdata = np.arange(100)
        dset = self.f.create_dataset(uid:=str(uuid4()), data=testdata, compression='lzf')
        self.assertEqual(dset.compression, 'lzf')
        self.assertEqual(dset.compression_opts, None)
    ...
    def test_lzf_exc(self):
        ...
        with self.assertRaises(ValueError):
            self.f.create_dataset(str(uuid4()), (20, 30), compression='lzf',
                                  compression_opts=4)
...
class TestCreateSZIP(BaseDataset):
    ...
    def test_szip(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), (20, 30), compression='szip',
...
class TestCreateShuffle(BaseDataset):
    ...
    def test_shuffle(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), (20, 30), shuffle=True)
...
class TestCreateFletcher32(BaseDataset):
    ...
    def test_fletcher32(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), (20, 30), fletcher32=True)
...
class TestCreateScaleOffset(BaseDataset):
    ...
    def test_float_fails_without_options(self):
        ...
        with self.assertRaises(ValueError):
            dset = self.f.create_dataset(str(uuid4()), (20, 30), dtype=float, scaleoffset=True)

    def test_non_integer(self):
        """ Check when scaleoffset is negetive"""

        with self.assertRaises(ValueError):
            dset = self.f.create_dataset(str(uuid4()), (20, 30), dtype=float, scaleoffset=-0.1)

    def test_unsupport_dtype(self):
        """ Check when dtype is unsupported type"""

        with self.assertRaises(TypeError):
            dset = self.f.create_dataset(str(uuid4()), (20, 30), dtype=bool, scaleoffset=True)
    ...
    def test_float(self):
        ...
        dset = self.f.create_dataset(uid:=str(uuid4()), shape, dtype=np.float64, scaleoffset=scalefac)
    ...
    def test_int(self):
        ...
        dset = self.f.create_dataset(uid:=str(uuid4()), shape, dtype=np.int64, scaleoffset=True)
    ...
    def test_int_with_minbits(self):
        ...
        dset = self.f.create_dataset(uid:=str(uuid4()), shape, dtype=np.int64, scaleoffset=nbits)
...
class TestExternal(BaseDataset):
    ...
    def test_contents(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), shape, dtype=testdata.dtype, external=external, efile_prefix="${ORIGIN}")
        dset[...] = testdata
    ...
    def test_contents_efile_prefix(self):
        ...
        dset = self.f.create_dataset(uid:=str(uuid4()), shape, dtype=testdata.dtype, external=external, efile_prefix=os.path.dirname(ext_file))
        dset[...] = testdata
        ...
        dset2 = self.f.require_dataset(uid, shape, testdata.dtype, efile_prefix=os.path.dirname(ext_file))
    ...
    def test_name_str(self):
        ...
        self.f.create_dataset(str(uuid4()), (6, 100), external=self.mktemp())

    def test_name_path(self):
        """ External argument may be a file name path only """

        self.f.create_dataset(str(uuid4()), (6, 100),
    ...
    def test_iter_multi(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), (6, 100), external=external)
    ...
    def test_invalid(self):
        ...
        for exc_type, external in [
            ...
            with self.assertRaises(exc_type):
                self.f.create_dataset(str(uuid4()), shape, external=external)
    ...
    def test_create_expandable(self):
        ...
        shape = (128, 64)
        maxshape = (None, 64)
        exp_dset = self.f.create_dataset(str(uuid4()), shape=shape, maxshape=maxshape,
                                         external=ext_file)
...
class TestAutoCreate(BaseDataset):
    ...
    def test_vlen_bytes(self):
        ...
        uid1 = str(uuid4())
        self.f[uid1] = b"Hello there"
        self.assert_string_type(self.f[uid1], h5py.h5t.CSET_ASCII)

        uid2 = str(uuid4())
        self.f[uid2] = [b"a", b"bc"]
        self.assert_string_type(self.f[uid2], h5py.h5t.CSET_ASCII)

        uid3 = str(uuid4())
        self.f[uid3] = np.array([b"a", b"bc"], dtype=np.object_)
        self.assert_string_type(self.f[uid3], h5py.h5t.CSET_ASCII)

    def test_vlen_unicode(self):
        """Assigning unicode strings produces a vlen string UTF-8 dataset """
        uid1 = str(uuid4())
        self.f[uid1] = "Hello there" + chr(0x2034)
        self.assert_string_type(self.f[uid1], h5py.h5t.CSET_UTF8)

        uid2 = str(uuid4())
        self.f[uid2] = ["a", "bc"]
        self.assert_string_type(self.f[uid2], h5py.h5t.CSET_UTF8)

        # 2D array; this only works with an array, not nested lists
        uid3 = str(uuid4())
        self.f[uid3] = np.array([["a", "bc"]], dtype=np.object_)
        self.assert_string_type(self.f[uid3], h5py.h5t.CSET_UTF8)

    def test_string_fixed(self):
        """ Assignment of fixed-length byte string produces a fixed-length
        ascii dataset """
        self.f[(uid:=str(uuid4()))] = np.bytes_("Hello there")
        ds = self.f[uid]
...
class TestCreateLike(BaseDataset):
    def test_no_chunks(self):
        uid1 = str(uuid4())
        self.f[uid1] = np.arange(25).reshape(5, 5)
        uid2 = str(uuid4())
        self.f.create_dataset_like(uid2, self.f[uid1])
        dslike = self.f[uid2]
        self.assertEqual(dslike.shape, (5, 5))
        self.assertIs(dslike.chunks, None)

    def test_track_times(self):
        orig = self.f.create_dataset(str(uuid4()), data=np.arange(12),
                                     track_times=True)
        self.assertNotEqual(0, h5py.h5g.get_objinfo(orig._id).mtime)
        similar = self.f.create_dataset_like(str(uuid4()), orig)
    ...
    def test_maxshape(self):
        ...
        other = self.f.create_dataset(str(uuid4()), (10,), maxshape=20)
        similar = self.f.create_dataset_like(str(uuid4()), other)
...
class TestChunkIterator(BaseDataset):
    ...
    def test_1d(self):
        dset = self.f.create_dataset(str(uuid4()), (100,), chunks=(32,))
    ...
    def test_2d(self):
        dset = self.f.create_dataset(str(uuid4()), (100,100), chunks=(32,64))
    ...
    def test_2d_partial_slice(self):
        dset = self.f.create_dataset(str(uuid4()), (5,5), chunks=(2,2))
...
class TestResize(BaseDataset):
    ...
    def test_resize_1D(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), 20, maxshape=40)
    ...
    def test_resize_nonchunked(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), (20, 30))
        with self.assertRaises(TypeError):
            dset.resize((20, 60))
    ...
    def test_zero_dim(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), (15, 0), maxshape=(15, None))
...
class TestDtype(BaseDataset):
    ...
    def test_dtype(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), (5,), '|S10')
    ...
    def test_dtype_complex32(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), (5,), complex32)
...
class TestLen(BaseDataset):
    ...
    def test_len(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), (312, 15))
        self.assertEqual(len(dset), 312)

    def test_len_big(self):
        """ Python len() vs Dataset.len() """
        dset = self.f.create_dataset(str(uuid4()), (2 ** 33, 15))
...
class TestIter(BaseDataset):
    ...
    def test_iter_scalar(self):
        ...
        dset = self.f.create_dataset(str(uuid4()), shape=())
        with self.assertRaises(TypeError):
            [x for x in dset]
...
class TestStrings(BaseDataset):
    ...
    def test_vlen_bytes(self):
        ...
        ds = self.f.create_dataset(str(uuid4()), (100,), dtype=dt)
        tid = ds.id.get_type()
        self.assertEqual(type(tid), h5py.h5t.TypeStringID)
    ...
    def test_vlen_bytes_fillvalue(self):
        ...
        dt = h5py.string_dtype(encoding='ascii')
        fill_value = b'bar'
        ds = self.f.create_dataset(uid:=str(uuid4()), (100,), dtype=dt, fillvalue=fill_value)
        self.assertEqual(self.f[uid][0], fill_value)
        self.assertEqual(self.f[uid].asstr()[0], fill_value.decode())
        self.assertEqual(self.f[uid].fillvalue, fill_value)
    ...
    def test_vlen_unicode_fillvalue(self):
        ...
        fill_value = 'bár'
        ds = self.f.create_dataset(uid:=str(uuid4()), (100,), dtype=dt, fillvalue=fill_value)
        self.assertEqual(self.f[uid][0], fill_value.encode("utf-8"))
        self.assertEqual(self.f[uid].asstr()[0], fill_value)
        self.assertEqual(self.f[uid].fillvalue, fill_value.encode("utf-8"))
    ...
    def test_fixed_utf_8_fillvalue(self):
        ...
        ds = self.f.create_dataset(uid:=str(uuid4()), (100,), dtype=dt, fillvalue=fill_value)
        self.assertEqual(self.f[uid][0], fill_value)
        self.assertEqual(self.f[uid].asstr()[0], fill_value.decode("utf-8"))
        self.assertEqual(self.f[uid].fillvalue, fill_value)
    ...
    def test_retrieve_vlen_unicode(self):
        dt = h5py.string_dtype()
        ds = self.f.create_dataset(str(uuid4()), (10,), dtype=dt)
        data = "fàilte"
        ds[0] = data
    ...
    def test_asstr(self):
        ds = self.f.create_dataset(str(uuid4()), (10,), dtype=h5py.string_dtype())
        data = "fàilte"
        ds[0] = data
...
class TestCompound(BaseDataset):
    ...
    def test_assign(self):
        ...
        ds = self.f.create_dataset(uid:=str(uuid4()), (16,), dtype=dt)
        for key in dt.fields:
            ds[key] = testdata[key]
    ...
    def test_fields(self):
        ...
        np.testing.assert_array_equal(
            self.f[uid].fields(['x', 'y'])[:], testdata[['x', 'y']]
        )
        # Extract single field
        np.testing.assert_array_equal(
            self.f[uid].fields('x')[:], testdata['x']
        )
        # Check __array__() method of fields wrapper
        np.testing.assert_array_equal(
            np.asarray(self.f[uid].fields(['x', 'y'])), testdata[['x', 'y']]
        )
        # Check type conversion of __array__() method
        dt_int = np.dtype([('x', np.int32)])
        np.testing.assert_array_equal(
            np.asarray(self.f[uid].fields(['x']), dtype=dt_int),
            testdata[['x']].astype(dt_int)
        )

        # Check len() on fields wrapper
        assert len(self.f[uid].fields('x')) == 16
...
class TestSubarray(BaseDataset):
    def test_write_list(self):
        ds = self.f.create_dataset(str(uuid4()), (1,), dtype="3int8")
        ds[0] = [1, 2, 3]
...
class TestEnum(BaseDataset):
    ...
    def test_create(self):
        ...
        ds = self.f.create_dataset(str(uuid4()), (100, 100), dtype=dt)
        dt2 = ds.dtype
...
class TestFloats(BaseDataset):
    ...
    def _exectest(self, dt):
        dset = self.f.create_dataset(str(uuid4()), (100,), dtype=dt)
...
class TestTrackTimes(BaseDataset):
    ...
    def test_disable_track_times(self):
        ...
        ds = self.f.create_dataset(str(uuid4()), (4,), track_times=False)
    ...
    def test_invalid_track_times(self):
        ...
        with self.assertRaises(TypeError):
            self.f.create_dataset(str(uuid4()), (4,), track_times='null')
...
class TestZeroShape(BaseDataset):
    ...
    def test_array_conversion(self):
        ...
        ds = self.f.create_dataset(str(uuid4()), 0, maxshape=None)
        self.assertEqual(ds.shape, np.array(ds).shape)

        ds = self.f.create_dataset(str(uuid4()), (0,), maxshape=(None,))
        self.assertEqual(ds.shape, np.array(ds).shape)

        ds = self.f.create_dataset(str(uuid4()), (0, 0), maxshape=(None, None))
        self.assertEqual(ds.shape, np.array(ds).shape)

    def test_reading(self):
        """ Slicing into empty datasets works correctly """
        dt = [('a', 'f'), ('b', 'i')]
        ds = self.f.create_dataset(str(uuid4()), (0,), dtype=dt, maxshape=(None,))
...
class TestRegionRefs(BaseDataset):
    ...
    def test_scalar_dataset(self):
        ds = self.f.create_dataset(str(uuid4()), data=1.0, dtype='f4')
...
class TestAstype(BaseDataset):
    ...
    def test_astype_wrapper(self):
        dset = self.f.create_dataset(str(uuid4()), (100,), dtype='i2')
        dset[...] = np.arange(100)
        arr = dset.astype('f4')[:]
...
class TestScalarCompound(BaseDataset):
    ...
    def test_scalar_compound(self):
        ...
        dt = np.dtype([('a', 'i')])
        dset = self.f.create_dataset(str(uuid4()), (), dtype=dt)
...
class TestVlen(BaseDataset):
    def test_int(self):
        ...
        ds = self.f.create_dataset(str(uuid4()), (4,), dtype=dt)
        ds[0] = np.arange(3)
        ds[1] = np.arange(0)
        ds[2] = [1, 2, 3]
        ds[3] = np.arange(1)
    ...
    def test_reuse_from_other(self):
        dt = h5py.vlen_dtype(int)
        ds = self.f.create_dataset(str(uuid4()), (1,), dtype=dt)
        self.f.create_dataset(str(uuid4()), (1,), ds[()].dtype)

    def test_reuse_struct_from_other(self):
        dt = [('a', int), ('b', h5py.vlen_dtype(int))]
        ds = self.f.create_dataset(uid:=str(uuid4()), (1,), dtype=dt)
        fname = self.f.filename
        self.f.close()
        self.f = h5py.File(fname, 'a')
        self.f.create_dataset(str(uuid4()), (1,), self.f[uid]['b'][()].dtype)

    def test_convert(self):
        dt = h5py.vlen_dtype(int)
        ds = self.f.create_dataset(str(uuid4()), (3,), dtype=dt)
    ...
    def test_multidim(self):
        dt = h5py.vlen_dtype(int)
        ds = self.f.create_dataset(str(uuid4()), (2, 2), dtype=dt)
        ds[0, 0] = np.arange(1)
    ...
    def _help_float_testing(self, np_dt, dataset_name):
        """
    ...
    def test_numpy_float16(self):
        ...
        self._help_float_testing(np_dt, dataset_name=str(uuid4()))
    ...
    def test_non_contiguous_arrays(self):
        ...
        self.f.create_dataset(uid1:=str(uuid4()), (10,), dtype=h5py.vlen_dtype('bool'))
        x = np.array([True, False, True, True, False, False, False])
        self.f[uid1][0] = x[::2]

        assert all(self.f[uid1][0] == x[::2]), f"{self.f[uid1][0]} != {x[::2]}"

        self.f.create_dataset(uid2:=str(uuid4()), (10,), dtype=h5py.vlen_dtype('int8'))
        y = np.array([2, 4, 1, 5, -1, 3, 7])
        self.f[uid2][0] = y[::2]

        assert all(self.f[uid2][0] == y[::2]), f"{self.f[uid2][0]} != {y[::2]}"

    def test_asstr_array_dtype(self):
        dt = h5py.string_dtype(encoding='ascii')
        fill_value = b'bar'
        ds = self.f.create_dataset(str(uuid4()), (100,), dtype=dt, fillvalue=fill_value)
...
class TestLowOpen(BaseDataset):
    ...
    def test_dapl(self):
        ...
        dset = self.f.create_dataset(uid:=str(uuid4()), (100,))
        del dset
        dsid = h5py.h5d.open(self.f.id, bytes(uid, 'utf-8'), dapl)
...
def test_get_chunk_details():
    ...
    with h5py.File(buf, 'w') as fout:
        fout.create_dataset(uid:=str(uuid4()), shape=(100, 100), chunks=(10, 10), dtype='i4')
        fout[uid][:] = 1
...
def test_chunk_iter():
    ...
    with h5py.File(buf, 'w') as f:
        f.create_dataset(uid:=str(uuid4()), shape=(100, 100), chunks=(10, 10), dtype='i4')
        f[uid][:] = 1
...
def test_empty_shape(writable_file):
    ds = writable_file.create_dataset(str(uuid4()), dtype='int32')
...
def test_zero_storage_size():
    ...
    buf = BytesIO()
    with h5py.File(buf, 'w') as fout:
        fout.create_dataset(uid:=str(uuid4()), dtype='uint8')

    buf.seek(0)
    with h5py.File(buf, 'r') as fin:
        assert fin[uid].chunks is None
        assert fin[uid].id.get_offset() is None
        assert fin[uid].id.get_storage_size() == 0
...
def test_python_int_uint64(writable_file):
    ...
    ds = writable_file.create_dataset(str(uuid4()), data=data, dtype=np.uint64)
...
def test_setitem_fancy_indexing(writable_file):
    ...
    arr = writable_file.create_dataset(str(uuid4()), (5, 1000, 2), dtype=np.uint8)
...
def test_allow_unknown_filter(writable_file):
    ...
    ds = writable_file.create_dataset(
        str(uuid4()), shape=(10, 10), dtype=np.uint8, compression=fake_filter_id,
...
def test_dset_chunk_cache():
    ...
    with h5py.File(buf, 'w') as fout:
        ds = fout.create_dataset(
            uid:=str(uuid4()), shape=(10, 20), chunks=(5, 4), dtype='i4',
...
class TestCommutative(BaseDataset):
    ...
    def test_numpy_commutative(self,):
        ...
        """
        shape = (100,1)
        dset = self.f.create_dataset(str(uuid4()), shape, dtype=float,
...
class TestVirtualPrefix(BaseDataset):
    ...
    def test_virtual_prefix_require(self):
        virtual_prefix = "/path/to/virtual"
        dset = self.f.require_dataset(str(uuid4()), (10, 3), 'f', virtual_prefix = virtual_prefix)
        virtual_prefix_readback = pathlib.Path(dset.id.get_access_plist().get_virtual_prefix().decode()).as_posix()
...
def ds_str(file, shape=(10, )):
    dt = h5py.string_dtype(encoding='ascii')
    fill_value = b'fill'
    return file.create_dataset(str(uuid4()), shape, dtype=dt, fillvalue=fill_value)
...
def ds_fields(file, shape=(10, )):
    ...
    file[(uid:=str(uuid4()))] = np.broadcast_to(fill_value, shape)
    return file[uid]
