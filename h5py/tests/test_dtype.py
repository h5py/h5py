...
class TestVlen(TestCase):
    ...
    def test_compound_vlen_bool(self):
        ...
        dt_vb = np.dtype([
            ('foo', vidt),
            ('logical', bool)])
        vb = f.create_dataset(name("dt_vb"), shape=(4,), dtype=dt_vb)
        ...
        dt_vv = np.dtype([
            ('foo', vidt),
            ('bar', vidt)])
        f.create_dataset(name("dt_vv"), shape=(4,), dtype=dt_vv)

        dt_vvb = np.dtype([
            ('foo', vidt),
            ('bar', vidt),
            ('logical', bool)])
        vvb = f.create_dataset(name("dt_vvb"), shape=(2,), dtype=dt_vvb)

        dt_bvv = np.dtype([
            ('logical', bool),
            ('foo', vidt),
            ('bar', vidt)])
        bvv = f.create_dataset(name("dt_bvv"), shape=(2,), dtype=dt_bvv)
    ...
    def test_compound_vlen_enum(self):
        ...
        dt_vve = np.dtype([
            ...
            ('bar', vidt),
            ('switch', eidt)])
        vve = f.create_dataset(name(), shape=(2,), dtype=dt_vve)
...
def test_opaque(writable_file):
    ...
    ds = writable_file.create_dataset(name(), data=arr)
