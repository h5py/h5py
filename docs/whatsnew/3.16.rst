...
* Support for storing NumPy complex numbers in HDF5 files using C99 complex
  ...
  :ref:`complex_dtypes` for how to use the new datatype.
...
* When creating a dataset, the ``dtype=`` parameter can now be a
  `TypeID <low:h5py.h5t.TypeID>` object from the low-level API.
...
* Fix building against pre-release version of NumPy (:pr:`2758`).
* Fixed some compiler warnings (:pr:`2784`, :pr:`2791`).
